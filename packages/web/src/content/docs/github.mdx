...
- `COMMAND`: Comma-separated list of trigger commands (default: `/opencode,/oc`). Example: `COMMAND: /cc`
- `AGENT`: Default agent to use if not mentioned in comment. Example: `AGENT: plan`
- `GIT_AUTHOR_NAME`: Global default git commit author name
- `GIT_AUTHOR_EMAIL`: Global default git commit author email
- `GIT_AUTHOR_NAME_<AGENT>`: Agent-specific git commit author name (e.g., `GIT_AUTHOR_NAME_PLAN`)
- `GIT_AUTHOR_EMAIL_<AGENT>`: Agent-specific git commit author email (e.g., `GIT_AUTHOR_EMAIL_PLAN`)
